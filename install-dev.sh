...
cat << 'EOF'
         ...
         CatStack — Mining Farm Manager  [DEV BUILD]
...
install_mfarm() {
    ...
    info "Installing mfarm from develop branch..."
    pip3 install --quiet --upgrade pip
    pip3 install --quiet "git+${MFARM_REPO}.git@${MFARM_BRANCH}" \
        || die "Failed to install mfarm. Try: pip3 install git+${MFARM_REPO}.git@${MFARM_BRANCH}"
    success "mfarm installed (develop)"
...
update_mfarm() {
    ...
    info "Updating mfarm to latest develop..."
    pip3 install --quiet --upgrade pip
    pip3 install --quiet --upgrade "git+${MFARM_REPO}.git@${MFARM_BRANCH}" \
        || die "Update failed. Try: pip3 install --upgrade git+${MFARM_REPO}.git@${MFARM_BRANCH}"
    success "mfarm updated to $(mfarm --version 2>/dev/null || echo 'unknown version') (develop)"
}
...
print_next_steps() {
    ...
    echo ""
    echo -e "  ${DIM}Running develop build — report issues at github.com/Meowcoin-Foundation/CatStack/issues${RESET}"
    echo ""
}

menu() {
    echo -e "${YELLOW}${BOLD}  ⚠  Development build — may be unstable${RESET}"
    echo ""
