...
log = logging.getLogger("mfarm-agent")
...
def query_ccminer_api(port: int) -> dict | None:
    """Query ccminer/cpuminer-opt via TCP API."""
    try:
        ...
        text = data.decode("utf-8", errors="replace").strip().rstrip("\0")
        if not text:
            return None
        ...
        return {
            "hashrate": float(kv.get("KHS", 0)) * 1000,  # Convert KH/s to H/s
...
class Agent:
    ...
    def run(self):
        log.info("MFarm Agent v%s starting on %s", VERSION, socket.gethostname())
        self.config.load()
