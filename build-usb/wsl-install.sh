#!/bin/bash
# Build MeowOS disk image using manual offsets (no partition nodes needed)
# Works around WSL2 not supporting partition scanning on loop OR passthrough disks
set -euo pipefail
...
echo "============================================"
echo "  MeowOS Image Builder (WSL2 compatible)"
echo "============================================"

# Find physical target
TARGET=""
for dev in /dev/sd?; do
    [ -b "$dev" ] || continue
    SIZE=$(blockdev --getsize64 "$dev" 2>/dev/null || echo 0)
    if [ "$SIZE" -gt 100000000000 ] && [ "$SIZE" -lt 130000000000 ]; then
        TARGET="$dev"
        echo "Physical target: $dev ($((SIZE / 1073741824)) GB)"
    fi
done
[ -z "$TARGET" ] && { echo "ERROR: No ~120GB disk found"; lsblk; exit 1; }

# Install tools
echo "Checking tools..."
apt-get update -qq
apt-get install -y -qq debootstrap gdisk dosfstools e2fsprogs grub-efi-amd64-bin 2>/dev/null

# [1/8] Create 8GB image
echo "[1/8] Creating 8GB disk image..."
rm -f "$IMG"
truncate -s 8G "$IMG"

# [2/8] Partition with fixed layout
...
# Known offsets (512-byte sectors):
#   Partition 1: start=2048, size=1048576 sectors = 512MB
#   Partition 2: start=1050624, to end of image
EFI_OFFSET=$((2048 * 512))          # 1048576
EFI_SIZE=$((1048576 * 512))          # 536870912
ROOT_OFFSET=$((1050624 * 512))       # 537919488

echo "  EFI:  offset=$EFI_OFFSET size=$EFI_SIZE"
echo "  Root: offset=$ROOT_OFFSET"

# [3/8] Create loop devices at specific offsets (NO partition scanning)
...
# [4/8] Mount + Debootstrap
echo "[4/8] Installing base system (~3 min)..."
mkdir -p "$MNT"
mount "$ROOT_LOOP" "$MNT"
...
# [6/8] MeowFarm agent + miners
echo "[6/8] Installing MeowFarm agent + miners..."
...
# [8/8] Write image to SSD
echo "[8/8] Writing image to $TARGET..."
dd if="$IMG" of="$TARGET" bs=4M status=progress oflag=sync
sync
rm -f "$IMG"
...
echo "============================================"
echo "  Plug SSD into rig and boot."
echo "  First boot expands to full 120GB + installs"
echo "  NVIDIA drivers, then starts mining."
echo "============================================"
